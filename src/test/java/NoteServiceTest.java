...
@Test
public void testRemoveNonExistentNote() {
    // Tworzymy notatkę, ale jej nie dodajemy do systemu
    Note note = new Note("Notatka, która nie istnieje");
    
    // Próbujemy usunąć notatkę, która nie została dodana
    boolean result = noteService.removeNote(note);
    
    // Sprawdzamy, czy wynik usuwania jest fałszywy
    assertFalse(result);
    
    // Sprawdzamy, czy lista jest nadal pusta
    assertEquals(0, noteService.getAllNotes().size());
}
